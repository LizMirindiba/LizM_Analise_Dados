...
import pandas as pd
df = pd.read_csv("/Users/lizmirindiba/Documents/Liz_analise_dados/Prova - Ap1/myntra_dataset_ByScraping.csv")
df = pd.read_csv("/Users/lizmirindiba/Documents/Liz_analise_dados/Prova - Ap1/ncr_ride_bookings.csv")
...
df_proporcao = len(df.loc[filtro]) / len(df)
df_proporcao
...
df_bike = df["Vehicle Type"] == "Bike"
filtro = df.loc[df_bike]
filtro["Payment Method"].value_counts()
...
df = pd.read_csv("/Users/lizmirindiba/Documents/Liz_analise_dados/Prova - Ap1/ncr_ride_bookings.csv")
df = pd.read_excel("/Users/lizmirindiba/Documents/Liz_analise_dados/Prova - Ap1/ncr_ride_bookings.xlsx")
filtro = df["Pickup Location"]
df_merged = pd.merge(df, regions, left_on="Pickup Location", right_on="Pickup Location")
...
import pandas as pd
import requests 
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNzYxMzYwMzgwLCJpYXQiOjE3NTg3NjgzODAsImp0aSI6IjI2MWQwMDQ3MDMwOTRmMWVhMDFhNDliODRlMTFjMjc1IiwidXNlcl9pZCI6Ijg3In0.DX8eXFrN_EgDvt4Z0Wslyy8y1f4XNeDwPAdFT5ma15M"
headers = {'Authorization': 'JWT {}'.format(token)}
params = {
    'data_base': '2023-01-02'
    }
...
Ativo = filtro = (df["descricao"] == "Ativo Total") 
Ativotivo= df.loc[Ativo,["valor"]].iloc[0]
Lucro = filtro = (df["descricao"] == "Lucro Liquido")
Lucro= df.loc[Ativo,["valor"]].iloc[0]
ROA = Lucro/Ativo
